...
| STT | Phân hệ / Hạng mục đánh giá | Điểm tối đa | Điểm tự đánh giá | Minh chứng & Báo cáo Deliverables |
|:---:|---|:---:|:---:|---|
| 1 | **API 1 (FR-02: Login & Lockout)** — Pipeline toàn diện (Generate + Audit + Extend + Execute + 4 Bugs) | 30 | 30 | `postman/`, `newman/member-1/fr02-report.html`, `test-cases/member-1.xlsx`, `reports/bug-report.md` |
| 2 | **API 2 (FR-07: Shopping Cart)** — Pipeline toàn diện (Generate + Audit + Extend + Execute + 4 Bugs) | 30 | 30 | `postman/`, `newman/member-1/fr07-report.html`, `test-cases/member-1.xlsx`, `reports/bug-report.md` |
| 3 | **API 3 (FR-15: Product CRUD)** — Pipeline toàn diện (Generate + Audit + Extend + Execute + 4 Bugs) | 30 | 30 | `postman/`, `newman/member-1/fr15-report.html`, `test-cases/member-1.xlsx`, `reports/bug-report.md` |
...
* Chi tiết 132 Test Cases được lưu trữ tại file Excel chuyên nghiệp: `test-cases/member-1.xlsx`.
...
### 3. Danh Sách Các Tính Năng Postman Đã Sử Dụng (Postman Features Utilized)
Theo yêu cầu tại Mục 6 & 14 đề bài, hệ thống kiểm thử đã ứng dụng toàn diện các tính năng nâng cao của Postman:
1. **Workspaces & Hierarchical Collections (v2.1.0 JSON):** Tổ chức cây thư mục 3 tầng, phân tách rành mạch Setup Auth, Sanity Suite và Bug Discovery Suite.
2. **Postman Environments & Variable Scoping:**
   - Quản lý môi trường độc lập: `HW06_Local.postman_environment.json` (Localhost:3000) và `HW06_Mock.postman_environment.json` (Mock Server).
   - Sử dụng biến động `{{base_url}}`, `{{student_id}}`, `{{admin_token}}`, `{{user_token}}`.
3. **Pre-request Scripts (Anti-Fraud & Dynamic Setup):**
   - Tự động gắn header chống gian lận `X-Student-Id: 23127205` trên 100% requests.
   - Dynamic token injection và khởi tạo timestamp ngẫu nhiên cho payload.
4. **Test Scripts với Chai Assertions & Ajv JSON Schema Validation:**
   - Kiểm tra mã HTTP status, cấu trúc Header `Content-Type: application/json`.
   - Kiểm tra hiệu năng: `pm.expect(pm.response.responseTime).to.be.below(500)`.
   - Giải mã Base64 JWT token để assert claims `role === 'admin'`.
   - Viết assertion phủ định bắt rò rỉ trường nhạy cảm: `pm.expect(jsonData.user).to.not.have.property('password')`.
5. **Data-Driven Testing (DDT via Collection Runner):**
   - Tích hợp 3 file dữ liệu ngoại vi CSV (`postman/data/fr02-login-data.csv`, `fr07-cart-data.csv`, `fr15-product-data.csv`) để chạy lặp tự động hàng chục kịch bản biên.
6. **Mock Server Configuration:** Cấu hình file `HW06_Mock` cho phép mô phỏng response API độc lập với backend SUT.
7. **Automated CLI Runner (Newman) & HTML Extra Reporting:** Tích hợp công cụ dòng lệnh Newman xuất báo cáo HTML trực quan cho CI/CD.

---

## V. TỔNG HỢP 12 LỖI & LỖ HỔNG BẢO MẬT SUT PHÁT HIỆN ĐƯỢC

Toàn bộ 12 lỗi được phân tích chi tiết kèm mã nguồn và đề xuất fix trong [reports/bug-report.md](bug-report.md):
...
- **Chi tiết báo cáo CI/CD:** [reports/cicd-report.md](cicd-report.md).
...
- **Đặc tả Skill:** [agent-skill/SKILL.md](../agent-skill/SKILL.md).
- **Mã nguồn CLI:** [agent-skill/generate_api_tests.py](../agent-skill/generate_api_tests.py).
- **Đặc tả chuẩn hóa đầu vào:** [docs/openapi.yaml](../docs/openapi.yaml).
